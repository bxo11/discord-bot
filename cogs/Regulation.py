...
class Regulation(commands.Cog):
    ...
    async def test(self, ctx):
        h = self.get_setting(699226343589806150, 'PopeChannel')
        self.set_setting(699226343589806150, 'PopeChannel', 'halo')

    def get_setting(self, guild_id: int, setting_name: str):
        session = Session()
        try:
            db_guild = session.query(models.Guilds).filter(models.Guilds.guild_id == guild_id).first()
            setting: models.Configuration = session.query(models.Configuration).filter(and_(models.Configuration.guild_id == db_guild.id), models.Configuration.setting_name == setting_name).first()
            session.close()
    ...
    def set_setting(self, guild_id: int, setting_name: str, new_value: str):
        session = Session()
        try:
            db_guild: models.Guilds = session.query(models.Guilds).filter(models.Guilds.guild_id == guild_id).first()
            setting: models.Configuration = session.query(models.Configuration).filter(
                and_(models.Configuration.guild_id == db_guild.id),
                models.Configuration.setting_name == setting_name).first()
